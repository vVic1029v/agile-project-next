...
export function collectTimeSlotRecords(
  ...
  // Initialize an empty record to store time slot records. if includesAllPeriods is true, just add keys for all periods.
  const timeslotRecords: Record<number, Record<number, TimeSlotCell>> = includeAllPeriods ? daySchedule.reduce((acc, _, i) => {
    acc[i] = {};
    return acc;
  }, {} as Record<number, Record<number, TimeSlotCell>>)
    : {};
...
export function getSelectedDateForDay(selectedDate: SelectedDate, dayWeek: number, timeslotPeriod: number): SelectedDate {
  const weekStartDate = getWeekStartDateFromYearWeek(selectedDate.year, selectedDate.week);
  return {
    ...selectedDate,
    day: weekStartDate.getDate() + dayWeek,
